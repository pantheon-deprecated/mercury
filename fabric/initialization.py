...
def _initialize_pressflow():
    ...
    run('git init /var/www/dev')
    run('git add /var/www/dev')
    run('git commit -m "initial checkin" /var/www/dev')
    sudo('git clone /var/www/dev /var/www/test')
    run('git checkout v1.0')
    sudo('mkdir /var/www/live')
    run('git archive master /var/www/test | sudo tar -x -C /var/www/live')
