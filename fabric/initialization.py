...
def _initialize_pressflow():
    ...
    run('cd /var/www/dev; git add .')
    run('cd /var/www/dev; git commit -m "initial checkin"')
    run('cd /var/www/dev; git tag v1.0')
    run('pwd')
    sudo('git clone /var/www/dev /var/www/test')
    run('cd /var/www/test; git checkout v1.0')
    sudo('mkdir /var/www/live')
    run('cd /var/www/dev; git archive master | sudo tar -x -C /var/www/live')
