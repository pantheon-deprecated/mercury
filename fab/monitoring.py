...
def check_load_average(limit):
    ...
    if (float(loads[0]) > float(limit)):
        jenkinstools.junit_fail('Load average is %s which is above the threshold of %s.' % (str(loads[0]), str(limit)), 'LoadAverage')
    else:
        jenkinstools.junit_pass('Load average is %s which is below the threshold of %s.' % (str(loads[0]), str(limit)), 'LoadAverage')
...
def check_disk_space(filesystem, limit):
    ...
    if (float(usage) > float(limit)):
        jenkinstools.junit_fail('Disk usage of %s is at %s percent which is above the threshold of %s percent.' % (filesystem, str(usage), str(limit)), 'DiskSpace') 
    else:
        jenkinstools.junit_pass('Disk usage of %s is at %s percent which is above the threshold of %s percent.' % (filesystem, str(usage), str(limit)), 'DiskSpace')
...
def check_swap_usage(limit):
    ...
    if (usage > float(limit)):
        jenkinstools.junit_fail('Swap usage is a %s percent which is above the threshold of %s percent.' % (str(usage), str(limit)), 'SwapUsage')
    else:
        jenkinstools.junit_pass('Swap usage is a %s percent which is below the threshold of %s percent.' % (str(usage), str(limit)), 'SwapUsage')


def check_io_wait_time(limit):
    iowait = local("vmstat | grep -v [a-z] | awk '{print $16}'").rstrip()
    if (float(iowait) > float(limit)):
        jenkinstools.junit_fail('IO wait times are at %s percent which is above the threshold of %s percent.' % (str(iowait), str(limit)), 'IOWaitTime')
    else:
        jenkinstools.junit_pass('IO wait times are at %s percent which is below the threshold of %s percent.' % (str(iowait), str(limit)), 'IOWaitTime')
...
def check_mysql(slow_query_limit, memory_usage, innodb_memory_usage, threads):
    with settings(warn_only=True):
        ...
        if report.failed:
            jenkinstools.junit_fail('mysql server does not appear to be running: %s' % report, 'MYSQL')
        ...
        if 'above' in message: 
            jenkinstools.junit_fail(message, 'MYSQLStatus')
        else:
            jenkinstools.junit_pass(message, 'MYSQLStatus')


def check_ldap():
    try:
        local('ldapsearch -H ldap://auth.getpantheon.com -x -ZZ')
        jenkinstools.junit_pass('ldap responded', 'LDAPStatus')
    except:
        jenkinstools.junit_error('Cannot connect to LDAP on localhost. \n%s' %
...
def check_pound_via_socket(port):
    ...
    try:
        ...
        s.shutdown(2)
        jenkinstools.junit_pass('pound responded', 'PoundSocket')
    except:
        jenkinstools.junit_error('Cannot connect to Pound on %s at %s. \n%s' % 
			        ('localhost', str(port), 
...
def check_memcached(port):
    ...
    try:
        ...
        s.connect(('localhost', port))
        s.shutdown(2)
        jenkinstools.junit_pass('memcached responded', 'MemcachedStatus')
    except:
        jenkinstools.junit_error('Cannot connect to Memcached on %s %s. \n%s' % 
				('localhost', str(port), 
...
def _test_url(service, url):
    ...
    if (status >=  400):
        jenkinstools.junit_fail('%s returned an error code of %s.' % (service, status), '%sStatus' % service)
    else:
        jenkinstools.junit_pass('%s returned an error code of %s.' % (service, status), '%sStatus' % service)
